                ...
                >
                  ...
                  <div style={{ padding: '1rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '2px', marginBottom: '0.5rem' }}>
                      {[1,2,3,4,5].map(i => <Star key={i} size={10} fill={i <= Math.round(p.avgRating || 0) ? '#F59E0B' : 'none'} color={i <= Math.round(p.avgRating || 0) ? '#F59E0B' : '#CBD5E1'} />)}
                      {p.totalReviews > 0 && <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)', marginLeft: '2px' }}>({p.totalReviews})</span>}
                    </div>
